...
class Generator:
    ...
    def __init__(self, FILE='symptoms', MIN_RANGE=2, MAX_RANGE=5, FREQUENCY=3, OUTCAST=10, MIN_OBSERVED=20,
        ...
        self.ratio = DAY_NIGHT_RATIO  # difference between quantity of observed at day/night time
    ...
    def gen_noice(self, data, group_data, percent):
        ...
        for group in data:
            ...
            for i in range(observes):
                ...
                MIN = self.min_range if self.max_range >= len(group) else len(group) - 1
                # quantity of features
                features = random.randrange(min(MIN, self.min_range), max(len(group), self.max_range))
    ...
    def run(self):
        count, data = self.load_lines(self.file)
    ...
    def export_to_json(self, filename, data):
        with open(filename, 'w', encoding='windows-1251') as fp:
            json.dump(data, fp, indent=4, ensure_ascii=False)

    def import_from_json(self, filename):
        with open(filename, 'r', encoding='windows-1251') as fp:
            file = json.loads(fp)
            data = json.dumps(file, sort_keys=False, indent=4, separators=(',', ': '))
            print(data)
        return data
